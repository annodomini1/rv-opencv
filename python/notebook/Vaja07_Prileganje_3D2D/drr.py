...
if __name__ == '__main__':
    ...
    #----------------------------------------------------------------------------------
    # Load the data
    ct = sitk.ReadImage("data/ct.nrrd")
    xray = sitk.ReadImage("data/xray.nrrd")
    ...
    if os.path.exists('data/drr.nrrd'):
        os.remove('data/drr.nrrd')
    sitk.WriteImage(sitk.GetImageFromArray(drr), 'data/drr.nrrd', True)

    if os.path.exists('data/drr_idx.nrrd'):
        os.remove('data/drr_idx.nrrd')
    sitk.WriteImage(sitk.GetImageFromArray(drr_idx), 'data/drr_idx.nrrd', True)
